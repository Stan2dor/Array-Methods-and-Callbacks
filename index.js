...
function getFinals(item) {
    const finalist = fifaData.filter(item => item.Stage === 'Final')
    return finalist;
}
...
function getWinners(fifaData) {
    ...
    winners = winners.map(function (item) {
        return item["Home Team Goals"] > item["Away Team Goals"] ? item["Home Team Name"] : item["Away Team Name"] // using the ternary operator to execute if the condition is true
    })
    return winners;
}
...
function getWinnersByYear(fifaData, getYears, getWinners) {
    ...
    let newArr = [];
    for (let i in getWinners(getYears)) {
...
function getAverageGoals(fifaData) {
    let totalNumber = fifaData.reduce(function (total, item) {
        return total + item["Home Team Goals"] + item["Away Team Goals"]
    }, 0)
    return (totalNumber / fifaData.length).toFixed(2);
}
